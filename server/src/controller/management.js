...
export const banUserDueToProfile = async (req, res) => {
  try {
    ...
    if (typeof banDays !== "undefined" && banDays !== null && Number(banDays) > 0) {
      ...
      // Thực hiện tăng tokenVersion + ban
      const updatedUser = await accountModel.findByIdAndUpdate(
        ...
        { new: true, select: "tokenVersion _id" }
      );
      if (updatedUser) {
        // Cập nhật redis tokenVersion đồng bộ
        await redis.set(`tokenVersion:${updatedUser._id}`, updatedUser.tokenVersion);
      }
      didBan = true;
    }
...
export const banUserAndRemovePost = async (req, res) => {
  ...
  try {
    ...
    if (typeof days !== "undefined" && Number(days) > 0) {
      ...
      // Cập nhật redis sau khi tăng tokenVersion
      if (banResult?._id && typeof redis !== "undefined" && redis.set) {
        try {
          await redis.set(`tokenVersion:${banResult._id}`, banResult.tokenVersion);
        } catch (e) {
          console.error("Không thể set redis key tokenVersion:", e);
        }
      }
...
export const banUserDueToMessage = async (req, res) => {
  try {
    ...
    if (Number(banDays) > 0) {
      ...
      // +1 vào tokenVersion đồng thời cập nhật redis
      if (typeof redis !== "undefined" && redis && typeof redis.set === 'function') {
...
export const banUser = async (req, res) => {
  try {
    ...
    if (Number(banDays) > 0) {
      ...
      // +1 vào tokenVersion đồng thời cập nhật redis
      if (user && typeof redis !== "undefined" && redis && typeof redis.set === 'function') {
        await redis.set(`tokenVersion:${user._id}`, user.tokenVersion);
      }
...
export const banUserWithReason = async (req, res) => {
  try {
    ...
    if (Number(banDays) > 0) {
      ...
      // đồng bộ redis tokenVersion nếu có cập nhật
      if (user && typeof redis !== "undefined" && redis && typeof redis.set === 'function') {
        await redis.set(`tokenVersion:${user._id}`, user.tokenVersion);
      }
    } else {
...
export const banUserAndRemoveComment = async (req, res) => {
  try {
    ...
    // Cập nhật người dùng và lấy bản ghi mới
    const updatedUser = await accountModel.findByIdAndUpdate(
      userId,
      {
        $set: update,
    ...
    // Set tokenVersion trong Redis theo yêu cầu
    if (updatedUser && typeof redis !== "undefined") {
      await redis.set(`tokenVersion:${updatedUser._id}`, updatedUser.tokenVersion);
    }
