...
import { apiGetGroupPost, apiUploadGroupPost } from "@/api/group.api";
...
export default function GroupPost({ groupId }: GroupPostProps) {
    ...
    // Hàm tăng/trừ số lượng bình luận mỗi khi có comment hoặc xóa comment, truyền vào Showpost
    ...
    // Fetch posts when groupId changes
    useEffect(() => {
        setLoadingPosts(true);
        setError('');
        apiGetGroupPost(groupId)
            .then((res: any) => {
                if (res?.data?.posts) {
                    ...
                    setError(res.data?.message || "Không tải được bài viết.");
                    setPosts([]);
                }
            })
            .catch((err) => {
    ...
    // Sửa handleOpenShowImage: truyền toàn bộ files + index + thông tin poster
    // avatar truyền vào ShowImage là avatar chưa crop
    ...
    // renderMedia truyền vào handleOpenShowImage với tất cả files và index click
    const renderMedia = (
        file: any,
        idx: number,
                ...
                onClick={() =>
                    handleOpenShowImage({
                        ...
                        avatar: post?.bioUser?.avatar
                            ? post.bioUser.avatar // DÙNG avatar chưa crop, truyền thẳng (KHÔNG CROP)
                            : undefined,
    ...
    // Sửa lại: khi upload post, server sẽ KHÔNG trả về name/avatar, tự patch vào newPost (myName/myAvatar/myUsername, myId)
    const handlePost = async (postData: PostData): Promise<PostResult> => {
        ...
        try {
            // GỌI API GROUP POST
            const res = await apiUploadGroupPost({ ...postData, groupId });
            if (res && res.data && res.data.success && res.data.post) {
                ...
                // Patch lại thông tin name/avatar từ redux vì server không trả về
                const newPost: PostType = {
                    ...
                    profileUser: {
                        ...(postFromApi.profileUser || {}),
                        name: myName,
                        username: myUsername
                    },
                    user: myId, // myId là userId của bài viết đó
    ...
    function getPostAvatar(post: PostType, size: number = 40): string {
        // Ưu tiên thông tin avatar tạm nếu mới đăng
        ...
        <div className="flex w-full max-w-6xl justify-center gap-6">
            ...
            {showImage && (
                <ShowImage
                    ...
                    avatar={showImageInfo.avatar}
                    avatarCroppedArea={showImageInfo.avatarCroppedArea}
                    name={showImageInfo.name}
                    username={showImageInfo.username}
                    createdAt={showImageInfo.createdAt}
                />
            )}
            ...
            {/* Added max-w-1/2 to make this block max-width 50% */}
                            ...
                            if (fileCount === 1) {
                                ...
                                mediaBlock = (
                                    <div className="mt-3 grid grid-cols-2 gap-2">
                                        {files.map((f, idx2) =>
                                            renderMedia(f, idx2, { className: "h-64" }, files, p)
                                        )}
                                    </div>
                                );
                            } else if (fileCount === 3) {
                                                                ...
                                                                onClick={() =>
                                                                    handleOpenShowImage({
                                                                        ...
                                                                        avatar: p.bioUser?.avatar
                                                                            ? p.bioUser.avatar // DÙNG avatar chưa crop ở đây
                                                                            : undefined,
                                    ...
                                    <div style={{ position: "relative" }}>
                                        {showFloating && (
                                            <div
                                                ...
                                                className={
                                                    `absolute top-10 z-3 w-[420px] bg-[#252728] border border-gray-600 rounded-xl flex-col transition-opacity duration-200 flex pointer-events-auto opacity-100`
                                                }
                                                style={{ left: 0 }}
                                        ...
                                        <div className="flex w-full">
                                            <div
                                                ...
                                                className={`flex items-center relative pr-2 z-2 max-w-full ${shouldUseHover ? "cursor-pointer" : ""}`}
