...
import { faFlag, faCrown, faVideo } from "@fortawesome/free-solid-svg-icons";
...
const ChatDock: React.FC<ChatDockProps> & { openChat?: Function } = ({
  ...
  // Removed minimize: no minimizedIds, minimizedUnread
  ...
  // Remove minimizedUnread, minimizedIds
  // Removed: const [minimizedUnread, setMinimizedUnread] = useState<Record<string | number, number>>({});
  // Removed: const [minimizedIds, setMinimizedIds] = useState<Array<ChatConversation["id"]>>([]);
  ...
  useEffect(() => {
    ...
    const off = listenMessages({
      onRoomMessage: async (msg: any) => {
        ...
        // Remove minimized logic: always just push to messages
        setPrivateMessages((prev) => ({
          ...prev,
          [peerId]: [
            ...(prev[peerId] || []),
      ...
      onPrivateMessage: async (msg: any) => {
        ...
        // Remove minimized logic. Always push to messages.
        setPrivateMessages((prev) => {
          const updated = {
            ...prev,
            [peerId]: [
  ...
  // Remove minimizedConversations and minimizedUnread related logic
  // Remove minimizedConversations const; not needed
  ...
  // Remove handleMinimize since we don't support minimize
  // Remove handleCloseAll/minimized logic referring to minimizedIds/read
  ...
  // Always show if there are any open conversations (no minimized logic)
              ...
              >
                ...
                <div className="flex items-center justify-between px-3 py-2 bg-[#1f2125] border-b border-[#2e3033]">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {/* <button ... onClick={() => handleMinimize(conv.id)} ... ></button> */}
                    <button
                      type="button"
                      className="flex items-center justify-center rounded-full border-none"
                      style={{
                ...
                >
                  ...
                  {tabLoading[conv.id] ? (
                    ...
                    <>
                      {/* ... Omit unchanged message rendering for brevity ... */}
                      {conv.type === "group"
                        ? (() => {
