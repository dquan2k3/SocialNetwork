...
const ChatDock: React.FC<ChatDockProps> & { openChat?: Function } = ({
  ...
  // PATCH: sẽ chạy hàm apiGetSelfChat và alert OK khi mở tab có type = self
  const onTabInitial = async (
    ...
    const isSelfTab =
      ...
      tabTypes[tabId] === "self" ||
      (() => {
        // Thử truy vấn trong dynamicConversations để nhận diện
        if (!dynamicConversations.some(dc => dc.id === "self" || dc.type === "self")) return false;
        if (tabId === "self") return true;
        // Nếu dynamicConversation với id này có type self
    ...
    if (!initialConv) {
      // If no initial conversation found, create a default initialConv object
      initialConv = {
        id: tabId,
              ...
              >
                ...
                >
                  {tabLoading[conv.id] ? (
                    ...
                    <>
                      {/* Render self messages just like private */}
                      {conv.type === "group"
                        ? (() => {
                          const bundles = (() => {
                            let bundles: Array<{
                        ...
                        : (() => {
                          // single-user chat: also for type=self!
