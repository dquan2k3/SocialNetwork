...
const ChatDock: React.FC<ChatDockProps> & { openChat?: Function } = ({
  ...
  // --- BẮT ĐẦU THAY ĐỔI: Listen for open tab with group type và chạy apiGetGroupUser ---
  // Khi tab được mở (handleOpenConversation, ở dòng setTabLoading), nếu là group: fetch và lưu group user
  // Đã có sẵn fetch ở onTabInitial nhưng ưu tiên high (ở groupPriority === "high" & nhận được message): gọi fetch group users nếu chưa có
  useEffect(() => {
    // Nếu tab đã mở và là group chat, fetch group users nếu chưa
    openIds.forEach(async (id) => {
      const convType = tabTypes[id] ||
        dynamicConversations.find((c) => c.id === id)?.type ||
        initialConversations.find((c) => c.id === id)?.type;
      if (convType === "group") {
        // Nếu chưa có groupUsersMap[id] hoặc bị rỗng, fetch group users với high priority
        if (!groupUsersMap[id] || groupUsersMap[id].length === 0) {
          const conversationId = tabConversationIds[id]
            || dynamicConversations.find((c) => c.id === id)?.conversationId
            || initialConversations.find((c) => c.id === id)?.conversationId
            || id;
          try {
            const result = await apiGetGroupUser(conversationId.toString());
  ...
  }, [openIds]); // chỉ chạy khi openIds thay đổi (mở tab)
  // --- KẾT THÚC BẮT ĐẦU THAY ĐỔI ---
  ...
  useEffect(() => {
    ...
    const off = listenMessages({
      onRoomMessage: async (msg: any) => {
        ...
        // ---- Thay đổi tại đây: Khi nhận được tin nhắn nhóm và mở group tab (high priority), fetch group users nếu chưa có ---
        const groupPri = groupPriority || "none";
        const isOpen = openIds.includes(peerId);

        // Nếu high priority và tab đã mở, fetch lại group users (luôn ưu tiên mới nhất)
        if (groupPri === "high" && isOpen) {
          try {
            // Ưu tiên high: fetch & update users
            const result = await apiGetGroupUser(groupConvId?.toString());
            if (result && Array.isArray(result.users)) {
              setGroupUsersMap((prev) => ({
                ...prev,
                [peerId]: result.users,
          ...
          setOpenIds((prev) => {
            ...
            if (!tabOpenedRef.current[peerId]) {
              ...
              onTabInitial(peerId, { type: "group", conversationId: groupConvId })?.then(async () => {
                // === Thêm: fetch group users ngay khi mở tab (high prio) ===
                try {
                  const result = await apiGetGroupUser(groupConvId?.toString());
                  if (result && Array.isArray(result.users)) {
                    setGroupUsersMap((prev) => ({
