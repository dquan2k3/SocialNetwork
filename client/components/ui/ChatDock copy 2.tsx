...
function MessageToast({ name, message, avatar, avatarCroppedArea, onClick }: MessageToastProps) {
  ...
  return (
    <div
      // Xóa sạch className, chuyển hết lớp sang style cho mẫu cơ bản
      style={{
        display: "flex",
        gap: 12,
        alignItems: "center",
    ...
    >
      ...
      >
        {avatar ? (
          ...
          >
            {/* Có thể thêm ký tự viết tắt nếu muốn, hiện tại để trống */}
          </div>
        )}
      </div>
...
const ChatDock: React.FC<ChatDockProps> & { openChat?: Function } = ({
  ...
  // Group cache for group conversations: key = conversationId, value = {conversationId, name, avatar}
  const [groupCache, setGroupCache] = useState<Record<string, { conversationId: string, name: string, avatar?: string }>>({});
  ...
  useEffect(() => {
    ...
    const off = listenMessages({
      onRoomMessage: async (msg: any) => {
        ...
        let groupInfo: { conversationId: string, name: string, avatar?: string } | undefined = undefined;
  ...
  const openConversations = useMemo(() => {
    ...
    const realConvs = openIds
      .map((id) => {
        ...
        // For group: prefer groupCache for name/avatar
        let title = c.title;
