...
import { apiGetGroupMedia } from "@/api/group.api";
...
export default function GroupMediaTab({ groupId }: GroupMediaTabProps) {
  ...
  useEffect(() => {
    ...
    apiGetGroupMedia(groupId)
      .then((res: any) => {
        if (cancelled) return;

        // NEW: Handle error message from server response (from getGroupMedia)
        if (res && res.success === false && typeof res.message === "string") {
  ...
  // Đặt width cố định cho vùng media để luôn sát lề trái, nếu ít item cũng không nằm giữa!
  const CONTAINER_WIDTH = 610; // match min width group content
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      {tab === "images" && (
        ...
        >
          <h3 className="w-full text-white text-3xl font-semibold mb-1 drop-shadow-lg tracking-wide">Ảnh</h3>
          {error ? (
            <div
              className="text-red-500 text-base font-medium px-4 py-2 h-12 flex items-center justify-center rounded-lg my-2 w-full text-center"
              style={{
                background: "#252728",
                border: "none",
              }}
            ...
            <div className="text-white text-lg font-medium animate-pulse">Đang tải ảnh...</div>
          ) : null}
      ...
      {tab === "videos" && (
        ...
        >
          <h3 className="w-full text-white text-3xl font-semibold mb-1 drop-shadow-lg tracking-wide">Video</h3>
          {error ? (
            <div
              className="text-red-500 text-base font-medium px-4 py-2 rounded-lg my-2 w-full text-center"
              style={{
                background: "#252728",
                border: "none",
              }}
            ...
            <div className="text-white text-lg font-medium animate-pulse">Đang tải video...</div>
          ) : null}
